document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to add a new debit row (only shows Asset and Expense accounts)
    function addDebitRow() {
        ...
        accounts.forEach(account => {
            // Only show Asset and Expense accounts in debit dropdown
    ...
    // Function to add a new credit row (only shows Liability, Revenue, and Equity accounts)
    function addCreditRow() {
        ...
        accounts.forEach(account => {
            // Only show Liability, Revenue, and Equity accounts in credit dropdown
            if (account.category === 'Liability' || account.category === 'Revenue' || account.category === 'Equity') {
                const option = document.createElement('option');
                option.value = account.no;
                option.textContent = `${account.no} - ${account.description}`;
                select.appendChild(option);
    ...
    function addEntry(event) {
        ...
        document.querySelectorAll('.debit-row').forEach(row => {
            ...
            if (accountNo && amount) {
                ...
                // Validate that debit accounts are only Asset or Expense
                if (account && (account.category !== 'Asset' && account.category !== 'Expense')) {
                    alert(`Error: Account ${accountNo} (${account.description}) is not a debit account (must be Asset or Expense)`);
                    debitError = true;
                    return;
                }
        ...
        document.querySelectorAll('.credit-row').forEach(row => {
            ...
            if (accountNo && amount) {
                ...
                // Validate that credit accounts are only Liability, Revenue, or Equity
                if (account && (account.category !== 'Liability' && account.category !== 'Revenue' && account.category !== 'Equity')) {
                    alert(`Error: Account ${accountNo} (${account.description}) is not a credit account (must be Liability, Revenue, or Equity)`);
                    creditError = true;
                    return;
                }
    ...
    function resetForm() {
        ...
        debitEntries.innerHTML = '<h4>Debit Entries (Assets/Expenses)</h4>';
        creditEntries.innerHTML = '<h4>Credit Entries (Liabilities/Revenue/Equity)</h4>';
        addDebitRow();
        addCreditRow();
        calculateTotals();
    }
    ...
    function updateAccountDropdowns() {
        ...
        // Update debit account dropdowns (only Asset and Expense)
        document.querySelectorAll('.debit-account').forEach(select => {
        ...
        // Update credit account dropdowns (only Liability, Revenue, and Equity)
        document.querySelectorAll('.credit-account').forEach(select => {
            const currentValue = select.value;
            const isRequired = select.required;
